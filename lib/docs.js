...
const chartsDirectory = path.join(docsDirectory, "charts"); // NEW
...
export async function getAllDocsSlugs() {
  ...
  // NEW — Charts directory, same pattern as components/blocks
...
export async function getDocBySlug(slug) {
  ...
  } else if (slug.startsWith("charts/")) {
    // NEW
    fullPath = path.join(chartsDirectory, `${slug.replace("charts/", "")}.mdx`);
