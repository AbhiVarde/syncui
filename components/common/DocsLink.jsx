...
const DocsLink = forwardRef(({ href, children, ...props }, ref) => {
  if (href?.startsWith("/docs")) {
    const handleClick = (e) => {
      ...
      if (isDocsSubdomain) {
        // If we're on docs subdomain, navigate to setup instead of index
        const path = href === "/docs" ? "/setup" : href.replace("/docs/", "/");
        window.location.href = path;
      } else {
        // If we're on main site, redirect to docs subdomain setup
        const path = href === "/docs" ? "setup" : href.replace("/docs/", "");
