---
title: Docks
description: Animated dock components for React. Use via CLI or copy the code.
---

Dock navigation built with Motion. Install via CLI or copy the code into your project.

#### Modern

Icons scale with a smooth gradient hover.
...
Icons grouped with dividers between sections.

<CodePreview
  preview={<DockVariants variant="categorical" />}
  cli="docks/categorical"
  code={`
import React, { useState } from "react";
...
Icons scale based on hover distance.
...
Icons sit on a frosted, translucent blur.
